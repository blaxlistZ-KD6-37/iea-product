...
debit_doc.classList.add("side-credit");
// Debit Datas
const debit_data = <HTMLDivElement>document.createElement("div");
debit_data.classList.add("data");
// Debits
...
let debit_summation = 0;
cash_debit.forEach((cash) => {
  debit_summation += cash.amount;
});

const debit_data_account = <HTMLDivElement>document.createElement("div");
debit_data_account.classList.add("d-acc_wrapper");
const debit_data_date = <HTMLTimeElement>document.createElement("time");
debit_data_date.classList.add("date");
const debit_formatted_date = ``;
debit_data_date.textContent = debit_formatted_date;
debit_data_date.dateTime = ``;
debit_data_account.appendChild(debit_data_date);

const debit_data_amount = <HTMLDivElement>document.createElement("div");
debit_data_amount.classList.add("date-acc-amount");
debit_data_amount.innerHTML = `&#x20B1;${debit_summation.toFixed(2)}`;
debit_data_account.appendChild(debit_data_amount);
...
const credit_formatted_date = `(${new Date(cash_credit[0].date)
  .toISOString()
  .slice(5, 7)}/${new Date(cash_credit[0].date).toUTCString().slice(5, 7)})`;
credit_data_date.textContent = credit_formatted_date;
...
console.log(credit_data_account);
console.log(debit_data_account);
